...
func placeWeapon(fromWho,weaponInstance,impulse,position) -> void:
	...
	if weaponInstance.position!=Vector3(fromWho.position.x,weaponInstance.position.y,fromWho.position.z):
		weaponInstance.look_at(Vector3(fromWho.position.x,weaponInstance.position.y,fromWho.position.z))
